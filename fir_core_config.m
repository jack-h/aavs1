...
function fir_core_config(this_block)
  ...
  dvld_port = this_block.port('dvld');
  dvld_port.setType('Bool');
  dvld_port.useHDLVector(false);

  % -----------------------------
  if (this_block.inputTypesKnown)
    % do input type checking, dynamic output type and generic setup in this code block.

    if (this_block.port('pol_in0').width ~= 32);
      this_block.setError('Input data type for port "pol_in0" must have width=32.');
    end

    if (this_block.port('pol_in1').width ~= 32);
      this_block.setError('Input data type for port "pol_in1" must have width=32.');
    end
    ...
    if (this_block.port('vld_in').width ~= 1);
      this_block.setError('Input data type for port "vld_in" must have width=1.');
    end

    this_block.port('vld_in').useHDLVector(false);
